...
class ByteStream {
    ...
    function popObject($type = 'uint8_t') {
    	...
    	if (substr($type, 0, 4) == 'uint' && class_exists($type,false)) {
    	...
    	// stl_map容器
    	elseif (substr($type, 0, 12) == 'stl_multimap') {
    		//var_dump('pop stl_map');
    		$type = str_replace(' ', '', $type);
    		$lt_pos = strpos($type, '<');
    		$gt_pos = strrpos($type, '>');
    		...
    		$multimap = new stl_multimap();
    		$multimap->setType($key_type, $value_type);
    		$object_value = $multimap->unserialize($this);
    		//var_dump('pop object=');
    		//print_r($object_value);
    		return $object_value;
    	}
    	...
    	// ICanSerializeObject
    	elseif (class_exists($type,false) && method_exists($type, 'unserialize')) {
