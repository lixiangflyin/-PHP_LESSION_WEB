...
class IAliAPI {
	...
	public static function response($params) {
		...
		$signType = empty($params['sign_type']) ? '' : $params['sign_type'];
		if ($signType != 'MD5') {
			self::setERR(8601, 'sign type err');
			return false;
		}
		...
		unset($params['mod'], $params['act'], $params['url'], $params['sign'], $params['sign_type']); //去掉多余的参数,准备计算签名
