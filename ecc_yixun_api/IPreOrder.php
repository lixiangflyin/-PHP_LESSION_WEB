...
class IPreOrder
{
	...
	/*
	* 调用说明
	* $offLine_params 中需要包含两个参数 type和items，type表示购物车类型，
	* 如果是0，则表示正常购物车，从用户的uid对应的购物车TTC中取数据
	* 如果不是0，则表示离线购物车，此时商品信息从items里面获得,items格式为json，示例
		 {"192244" : {
				 "product_id" : 192244,
				 "buy_count" : 1,
				 "main_product_id" : 0,
    ...
    */
	public static function listPackageInShoppingCart($uid, $wh_id, $district, $rules = array(), $offLine_params = array())
	{
		...
		// 获取购物车中的商品，包括商品ID，购买数量，
		$ret = self::getItemList($uid, $wh_id, $offLine_params); //拉取购物车
		if (false === $ret) {
			return false;
		}
		...
		// 根据站点，赠品，库存的信息，items里面的商品信息需要变化
		...
		//重新取pid，中间的操作过程，可能把某些pid给删除掉了，所以凡是引起items发生变化的过程，请在这之前调用
		...
		if ($shopping_cart_type != IShoppingCart::INSTALLMENT_CART) {
			...
			if (false === $promoCoupon) {
				...
				self::$logMsg = basename(__FILE__) . "line:" . __LINE__ . ",errMsg:查询试用的返券信息失败，" . EA_Promotion::$errMsg;
			}
		}
	...
	/**
	 * 获取商品信息，不带包裹信息，不包含拆单(cart)
	 * @param int $uid 用户ID
	 * @param int $wh_id 分站ID
	 * @param int $district 所在地区ID
	...
	public static function listProductsInfo($uid, $wh_id, $district, $offLine_params) {
		...
		if (false === $products) {
			self::$errCode = IProduct::$errCode;
			self::$errMsg = basename(__FILE__) . ",line:" . __LINE__ . ",errMsg:获取商品信息失败" . IProduct::$logMsg;
			return false;
		}
