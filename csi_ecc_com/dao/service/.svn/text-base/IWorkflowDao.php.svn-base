...
class IWorkflowDao
{
	...
	public static function insert($data) {
		...
		$v = $db->insert('spf_workflows', $data);
		if($v === false) {
	...
	public static function update($data, $where)
	{
		...
		$v = $db->update('spf_workflows', $data, $where);
		if($v === false) {
			self::$errCode = $db->errCode;
			self::$errMsg  = $db->errMsg;
			return false;
	...
	public static function remove($where)
	{
		...
		$v = $db->remove('spf_workflows', $where);
		if($v === false) {
			self::$errCode = $db->errCode;
			self::$errMsg  = $db->errMsg;
			return false;
	...
	public static function getList($fields, $where, $start, $length)
	{
		...
		$v = $db->getRows2('spf_workflows', $fields, $where, $start, $length, ' create_time asc ');
		if($v === false) {
			self::$errCode = $db->errCode;
			self::$errMsg  = $db->errMsg;
			echo self::$errMsg;
		...
		foreach ($v as &$row) {
			if (!empty($row['workflow_detail'])) {
				$row['workflow_detail'] = iconv("gbk", "utf8", $row['workflow_detail']);
			}
		}
		unset($row);
		return $v;
	 ...
	 */
	public static function getWorkflows($complaint_id) {
		self::clearErr();
		$db = Config::getDB('b2b2c_kf_admin');
		if(!$db) {
			self::$errCode = Config::$errCode;
		...
		$sql = "SELECT * FROM spf_workflows WHERE complaint_id=" . intval($complaint_id) . " ORDER BY create_time ASC";
		$v = $db->getRows($sql);
		if($v === false) {
			self::$errCode = $db->errCode;
			self::$errMsg  = $db->errMsg;
	...
	public static function getCount($where)
	{
		...
		$v = $db->getRowsCount('spf_workflows', $where);
		if($v === false) {
			self::$errCode = $db->errCode;
			self::$errMsg  = $db->errMsg;
			return false;
