...
dealUnassign.bindInitEvent = function(){
	var _self = this;
	$('.up_down').click(
		function() {
	...
	$("#result_export").click(function() {
		url = '/json.php?biz=service&mod=mywork&act=unassignExport';
		var data = {
...
dealUnassign.bindSubmitEvent = function(){
	var _self = this;
...
dealUnassign.search = function(data) {
	var url = '';
	var fields = new Array();
	$("#fields_checkbox input[type='checkbox']:checked").each(function() {
	 	fields.push($(this).val());
	...
	url = '/json.php?biz=service&mod=mywork&act=groupunassign';
	CSI.loadingShow();
	$.ajax({
			url	 : url,
			type 	 : 'POST',
			...
			success  : function(data){
				if(0 == data.errno){
					dealUnassign._render_data(data.data);
				}else{	
					alert("系统问题，请重新查询");
				}
			}
...
dealUnassign._render_data = function (data) {
	CSI.loadingHide();
	...
	if(data.groups.length > 0) {
		var pre_group_id = $("#group_id").val();
		var option_html = '<option value="">全部</option>';
		for(var i = 0; i < data.groups.length; i ++){
			option_html += '<option value="' + data.groups[i].gid + '" ' + (pre_group_id == data.groups[i].gid ? ' selected ' : '')+ '>' + data.groups[i].name  + '</option>'
		}
		$("#group_id").empty().append(option_html);
	}
				...
				switch (k) {
					...
					case 'type':
						if (data.list[i][k] > 100) {
							html += '<td>' + dealUnassign.APPLY_SUBTYPE[data.list[i][k]] + '</td>';
						} else {
							html += '<td>' + dealUnassign.APPLY_TYPE[data.list[i][k]] + '</td>';
						}
						break;
					case 'state':
						html += '<td>' + dealUnassign.APPLY_STATE[data.list[i][k]] + '</td>';
						break;
					case 'checkup':
						html += '<td>' + dealUnassign.APPLY_CHECKUP[data.list[i][k]] + '</td>';
						break;
					case 'source':
						html += '<td>' + dealUnassign.APPLY_SOURCE[data.list[i][k]] + '</td>';
						break;
					case 'approve':
						html += '<td>' + dealUnassign.APPLY_APPROVE[data.list[i][k]] + '</td>';
						break;
					case 'isVip':
						html += '<td>' + (data.list[i][k] > 0 ?'是' : '否') + '</td>';
						break;
					case 'createTime':
						html += '<td>' + dealUnassign.timeFormat(data.list[i][k],'y-m-d h:i:s') + '</td>';
						break;
					case 'finishTime':
					case 'nextTime':
					case 'acceptTime':
	...
	var paginator_html = getPageHTML("javascript:dealUnassign.goPage({page})", data.page, data.pages, 5);
	$("#paginator").empty().html(paginator_html);
}
dealUnassign.assign = function() {
	var ids = new Array();
		$("#search_result_table input[type='checkbox']").each(function() {
			if ($(this).attr('checked') == 'checked') {
				ids.push($(this).val());
...
dealUnassign.goPage = function(page) {
	 var data = {
	...
	dealUnassign.search(data);
};


dealUnassign.getCookie = function(name) {
	//读取COOKIE
...
dealUnassign.getUsername = function(){
	//返回当前登陆用户的用户名 ,没有则返回""
	var username = this.getCookie("username");
	return username ? username : "";
};

dealUnassign.strToDate = function(str){
	if(!str) return null;
 ...
 * 时间戳转换成时间对象
 */
dealUnassign.timeFormat= function(ts, fstr) {
...
dealUnassign.QueryString = function(item){
  var svalue = window.location.search.match(new RegExp("[\?\&]" + item + "=([^\&]*)(\&?)","i"));
  return svalue ? svalue[1] : svalue;
}

dealUnassign.getInfo = function(){
	var getUrl = '/json.php?biz=service&mod=mywork&act=groupunassign';
...
dealUnassign.getTimeInfo = function(t) {
