...
dealUnsolved.bindInitEvent = function(){
	var _self = this;
	$('.up_down').click(
		function() {
	...
	$("#result_export").click(function() {
		url = '/json.php?biz=service&mod=mywork&act=unsolvedExport';
		var data = {
	...
	$("#reassign_submit_btn").click(function(){
		...
		if(ids.length < 1) {
			...
			if (!data.assign_target) {
				$("#reassign_zone .msg-tips").html("请输入转单人！");
				return false;
			}
			$.ajax({
				url	 : url,
...
dealUnsolved.bindSubmitEvent = function(){
	var _self = this;
...
dealUnsolved.search = function(data) {
	var url = '';
	var fields = new Array();
	$("#fields_checkbox input[type='checkbox']:checked").each(function() {
	 	fields.push($(this).val());
	...
	url = '/json.php?biz=service&mod=mywork&act=unsolved';
	...
	$.ajax({
			...
			success  : function(data){
				if(0 == data.errno){
					dealUnsolved._render_data(data.data);
				}else{	
					alert("系统问题，请重新查询");
				}
			}
...
dealUnsolved._render_data = function (data) {
	CSI.loadingHide();
				...
				switch (k) {
					...
					case 'type':
						if (data.list[i][k] > 100) {
							html += '<td>' + dealUnsolved.APPLY_SUBTYPE[data.list[i][k]] + '</td>';
						} else {
							html += '<td>' + dealUnsolved.APPLY_TYPE[data.list[i][k]] + '</td>';
						}
						break;
					case 'flag':
						...
						break;
					case 'state':
						html += '<td>' + dealUnsolved.APPLY_STATE[data.list[i][k]] + '</td>';
						break;
					case 'checkup':
						html += '<td>' + dealUnsolved.APPLY_CHECKUP[data.list[i][k]] + '</td>';
						break;
					case 'source':
						html += '<td>' + dealUnsolved.APPLY_SOURCE[data.list[i][k]] + '</td>';
						break;
					case 'approve':
						html += '<td>' + dealUnsolved.APPLY_APPROVE[data.list[i][k]] + '</td>';
						break;
					case 'isVip':
						html += '<td>' + (data.list[i][k] > 0 ?'是' : '否') + '</td>';
						break;
					case 'createTime':
						html += '<td>' + dealUnsolved.timeFormat(data.list[i][k],'y-m-d h:i:s') + '</td>';
						break;
					case 'finishTime':
					case 'nextTime':
					case 'acceptTime':
	...
	var paginator_html = getPageHTML("javascript:dealUnsolved.goPage({page})", data.page, data.pages, 5);
	$("#paginator").empty().html(paginator_html);
}

dealUnsolved.goPage = function(page) {
	 var data = {
		//type: $("#unsolved_filter_type").val(),
		state: $("#unsolved_filter_state").val(),
		expire: $("#unsolved_filter_expire").val(),
	...
	dealUnsolved.search(data);
};


dealUnsolved.getCookie = function(name) {
	//读取COOKIE
...
dealUnsolved.getUsername = function(){
	//返回当前登陆用户的用户名 ,没有则返回""
	var username = this.getCookie("username");
	return username ? username : "";
};

dealUnsolved.strToDate = function(str){
	if(!str) return null;
 ...
 * 时间戳转换成时间对象
 */
dealUnsolved.timeFormat= function(ts, fstr) {
...
dealUnsolved.QueryString = function(item){
  var svalue = window.location.search.match(new RegExp("[\?\&]" + item + "=([^\&]*)(\&?)","i"));
  return svalue ? svalue[1] : svalue;
}

dealUnsolved.getInfo = function(){
	CSI.loadingShow(true);
	var getUrl = '/json.php?biz=service&mod=mywork&act=unsolved';
...
dealUnsolved.getTimeInfo = function(t) {
