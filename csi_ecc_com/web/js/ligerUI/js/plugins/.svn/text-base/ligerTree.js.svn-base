...
$.fn.ligerTree = function (p)
{
    ...
    getParentTreeItem: function (treenode, level)
  {
    ...
    loadData: function (node, url,param)
    {
      ...
      if (p.onSuccess) p.onSuccess(newdata);
      },
    ...
    clickfather:function(treeNode)
    {
      var parentNode = g.getParentTreeItem(treeNode);
      if(parentNode!= undefined)
      {
        g.clickfather(parentNode);
      }

      var treedataindex = parseInt($(treeNode).attr("treedataindex"));
      $(treeNode).trigger('click');
    },
	GetDataByHtml:function(treeNode)
			{
				var treedataindex = parseInt($(treeNode).attr("treedataindex"));
				return po.getDataNodeByTreeDataIndex(g.data,treedataindex);
			},
