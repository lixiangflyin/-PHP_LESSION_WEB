...
class IReview
{
    ...
    protected static function sendRequest($cmd_, $is_get_ = false)
    {
        ...
        {
            self::$errMsg = basename(__FILE__, '.php') . " |" . __LINE__ . "[review system svr timeout]";
            return  false;
        }
    ...
    protected static function synReviewToERP($review_info_)
    {
        ...
        $info = IUsersTTC::get($review_info_['user_id']);
        $uid = $review_info_['user_id'];
        if ( false === $info || count($info) != 1)
        {
            self::$errCode = 24;
            self::$errMsg = basename(__FILE__, '.php') . " |" . __LINE__ . "query $uid from userTTC faild:IUsersTTC::errCode";
        }
        else
        {
            $level = $info[0]['level'];
    ...
    protected static function synReplyToERP($reply_info_)
    {
        ...
        $info = IUsersTTC::get($reply_info_['user_id']);
        if ( false === $info || count($info) != 1)
        {
            self::$errCode = 24;
            self::$errMsg = basename(__FILE__, '.php') . " |" . __LINE__ . "query {$reply_info_['user_id']} from userTTC faild:IUsersTTC::errCode";
        }
        else
        {
            $level = $info[0]['level'];
    ...
    protected static function getScore($user_id_, $price_)
    {
        ...
        //get user level
        $item = IUsersTTC::get($user_id_, array(), array('level', 'type'));
        if (false == $item || count($item) <= 0)
        {
            self::$errMsg = basename(__FILE__, '.php') . " |" . __LINE__ . " get ($user_id_) from usersTTC faild.";
            return  false;
        }
        ...
        if ( in_array($item[0]['type'], $no_score_user_type) ) { //炒货商,经销商 ,分销商 企业用户
